from pyspark.sql.functions import col, lag, avg, stddev, log
...
def create_gold_analytics() -> None:
    ...
    # Sector 30-day volatility of daily returns （Personally added it)
    w = Window.partitionBy("sector").orderBy("date").rowsBetween(-29, 0)

    sector_vol_30d = (
    df.withColumn("volatility_30d", stddev("daily_return").over(w))
    .select("date", "sector", "volatility_30d")
    )
    
    sector_daily.write.mode("overwrite").parquet(f"{p.GOLD_ANALYTICS_DIR}/sector_daily_avg_return")
    sector_vol_30d.write.mode("overwrite").parquet(f"{p.GOLD_ANALYTICS_DIR}/sector_30d_volatility")
