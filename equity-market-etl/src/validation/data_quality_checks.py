...
def silver_checks() -> None:
    ...
    #Cannot deal with date & string type column isnan, so we check date column separately below
    # for c in ["date", "ticker", "close", "adj_close", "volume"]:
    #     _fail_if(prices, col(c).isNull() | isnan(col(c)), f"[silver_prices] Null/NaN found in {c}")
    
    # Divide columns into "numeric columns that need NaN checks" and "regular columns that only need Null checks"
    numeric_cols = ["close", "adj_close", "volume"]
    categorical_cols = ["date", "ticker"]

    # 1. Check date and ticker columns (only check for Null)
    for c in categorical_cols:
        _fail_if(prices, col(c).isNull(), f"[silver_prices] Null found in {c}")

    # 2. Check price and volume columns (check for both Null and NaN)
